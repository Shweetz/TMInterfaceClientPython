...
from read_state import load_state
rules = []
...
# rules.append(Rule(Input.UP___, Change.TIMING, proba=0.02, start_time=start, end_time=end, diff=30))
# rules.append(Rule(Input.DOWN_, Change.TIMING, proba=0.02, start_time=start, end_time=end, diff=30))
...
POINT_POS = [495, 73, 80] # multilap
POINT_POS = [545, 141, 489]
POINT_POS = [530, 57, 435]
POINT_POS = [530, 57, 443]
...
p = 0
steer_equal_last_input_proba = 0.1 # proba to make a steer equal to last steer
...
class MainClient(Client):
    ...
    def on_simulation_begin(self, iface):
        ...
        if LOAD_INPUTS_FROM_FILE:
            ...
            if not LOAD_REPLAY_FROM_STATE:
                iface.set_event_buffer(self.begin_buffer) # COMMENT FOR PARTIAL BUFFER AND SAVE STATES
    ...
    def is_custom(self, base_run, min_diff=0):
        ...
        #     return False
        # # print(self.car.y)
        # if not self.car.get_speed("xyz") > 450:
        #     return False
        # if not 57 < self.car.y:
        #     return False
        # if not 444 > self.car.z:
        #     return False
        ...
        self.car.custom = self.car.vel_z * 3.6
        # self.car.custom = get_dist_2_points(POINT_POS, self.car.position, "xz")
        self.car.custom = -self.car.vel_x +self.car.vel_z - self.car.x + self.car.z
        # self.car.custom = self.car.get_speed("xyz") - math.sqrt(get_dist_2_points(POINT_POS, self.car.position))
        #self.car.custom = - self.car.vel_z - self.car.z
        
        self.car.custom = self.car.y

        # cx = -5
        # cz = 1
        # cspeed = 1
        # x = self.car.x + self.car.vel_x*cspeed
        # z =   self.car.vel_z*cspeed
        # self.car.custom = x*cx + z*cz
        
        if base_run:
            print(f"Base run custom = {self.car.custom}")
            return True
        elif self.car.custom > self.best_car.custom + min_diff and self.car.custom > 88.05:
