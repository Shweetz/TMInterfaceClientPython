...
class Change:
    ...
    def __str__(self):
        return f"rule: From {self.start_time} to {self.end_time}ms, change {self.change_type.name} for {self.input} with max value of {self.value} and modify_prob={self.proba}"
...
rules.append(Change(ANALOG_STEER_NAME, ChangeType.STEER_DIFF,     proba=0.1, start_time=4000, end_time=4390, value=65536))
