        ...
        if self.simtime == -10:
            ...
            if curr_vel < self.last_vel - 0.2 and self.simtime > 1000:
                ...
                if self.working.max_steer - self.working.min_steer == 1:
                    if self.working.min_crash > self.working.max_crash:
                        self.working.cur_steer = self.working.min_steer
                    else:
                        self.working.cur_steer = self.working.max_steer
                    print(f"Perfect: {self.working._time} steer {self.working.cur_steer}")
                    self.steer_event(iface)
